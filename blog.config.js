...
const BLOG = {
  ...
  NEXT_REVALIDATE_SECOND: 10, // 更新内容缓存间隔 单位 (秒)；即每个页面有 5 秒的纯静态期、此期间无论多少次访问都不会抓取 notion 数据；调大该值有助于节省 Vercel 资源、同时提升访问速率，但也会使文章更新有延迟。
  ...
  APPEARANCE: 'dark', // ['light', 'dark', 'auto'], // light 日间模式，dark 夜间模式，auto 根据时间和主题自动夜间模式
  ...
  CUSTOM_FONT_URL: ['https://npm.elemecdn.com/lxgw-wenkai-webfont@1.6.0/style.css'], // 自定义字体的 CSS
  ...
  BEI_AN: process.env.NEXT_PUBLIC_BEI_AN || '萌 ICP 备 20224002 号', // 备案号 闽 ICP 备 XXXXXXX
  ...
  POST_URL_PREFIX: process.env.NEXT_PUBLIC_POST_URL_PREFIX || 'post', // POST 类型文章的默认路径前缀，例如默认 POST 类型的路径是  /article/[slug]
  ...
  POSTS_SORT_BY: 'date', // 排序方式 'date'按时间，'notion'由 notion 控制
  ...
  COMMENT_GISCUS_REPO: process.env.NEXT_PUBLIC_COMMENT_GISCUS_REPO || 'IoriMaboroshi/NotionNext', // 你的 Github 仓库名 e.g 'tangly1024/NotionNext'
  COMMENT_GISCUS_REPO_ID: process.env.NEXT_PUBLIC_COMMENT_GISCUS_REPO_ID || 'R_kgDOIjHvBg', // 你的 Github Repo ID e.g ( 設定完 giscus 即可看到 )
  COMMENT_GISCUS_CATEGORY_ID: process.env.NEXT_PUBLIC_COMMENT_GISCUS_CATEGORY_ID || 'DIC_kwDOIjHvBs4CS5T5', // 你的 Github Discussions 內的 Category ID ( 設定完 giscus 即可看到 )
  COMMENT_GISCUS_MAPPING: process.env.NEXT_PUBLIC_COMMENT_GISCUS_MAPPING || 'pathname', // 你的 Github Discussions 使用哪種方式來標定文章，預設 'pathname'
  ...
  COMMENT_GITALK_CLIENT_ID: process.env.NEXT_PUBLIC_COMMENT_GITALK_CLIENT_ID || '', // e.g 20 位 ID，在 gitalk 后台获取
  COMMENT_GITALK_CLIENT_SECRET: process.env.NEXT_PUBLIC_COMMENT_GITALK_CLIENT_SECRET || '', // e.g 40 位 ID，在 gitalk 后台获取
  ...
  COMMENT_WALINE_SERVER_URL: process.env.NEXT_PUBLIC_WALINE_SERVER_URL || 'https://waline.chwin.asia/', // 请配置完整的 Waline 评论地址 例如 hhttps://preview-waline.tangly1024.com @see https://waline.js.org/guide/get-started.html
  ...
  ANALYTICS_GOOGLE_ID: process.env.NEXT_PUBLIC_ANALYTICS_GOOGLE_ID || 'G-K4MJ3FFGEE', // 谷歌 Analytics 的 id e.g: G-XXXXXXXXXX
  ...
  // 作废配置
  AVATAR: 'https://b.chwin.asia/wp-content/uploads/2022/11/cropped-lab.magiconch.com90s-time-machine-1667800846069.jpg', // 作者头像，被 notion 中的 ICON 覆盖。若无 ICON 则取 public 目录下的 avatar.png
  TITLE: '话痨小幻', // 站点标题，被 notion 中的页面标题覆盖
  HOME_BANNER_IMAGE: './bg_image.jpg', // 首页背景大图，会被 notion 中的封面图覆盖，若无封面图则会使用代码中的 /public/bg_image.jpg 文件
  DESCRIPTION: '幻幻的日常', // 站点描述，被 notion 中的页面描述覆盖
